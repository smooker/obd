...
## USB transport layer (потвърдено 2026-04-29 от binary capture)

DS150E има два bulk IN endpoints:
- **EP 0x81** — FTDI modem status, само 2 байта: `\x11\x60`. Пропускаме.
- **EP 0x82** — реални ECU данни (serial от STM32). Четем от тук.

usbmon binary header: **48 байта** (`struct usbmon_packet` от Linux kernel).
Форматът на файла: `[48-byte header][cap_len bytes payload]` на пакет.

### ECU response structure (EP 0x82)

```
[0x00][0x21][len][...][0x09][0xEF]*\x00*<decimal ASCII bytes>\r
                                         └─────────────────────┘
                                         Текстов отговор (ASCII)
```

Текстовата секция след `*\x00*`:
```
*97 <idx_dec> <byte1_dec> <byte2_dec> ... <byteN_dec>\r   ← стандартен param
```

**За B0-B9** (специален формат — idx + 2 стойности с 0x01 сепаратор):
```
<idx_dec> 1 <A_hi_dec> 1 <A_lo_dec>\r
```
→ Декодира се: `(A_hi*256 + A_lo) / 480 = °C`
...
| 08 | `00 00 D0 00 7F` | ? |
| 09 | `B3 0F 00 00` | ? |
| 10 | `byte[0] byte[1] byte[2] byte[3] A0` | **DPF диф. налягане + RPM** (виж долу) |
...
#### Param 0x10 — декодиране (потвърдено 2026-04-29)

Format: 5 байта = `[P] [?] [?] [RPM] [A0]`

| byte | диапазон | формула | единица |
|------|----------|---------|---------|
| [0] | 64–71+ | **(byte[0] - 64) / 3** | kPa (DPF диф. налягане) |
| [1] | 64–70 | ? | ? |
| [2] | 65–71 | ? | ? |
| [3] | 20–31 | **byte[3] × 100** | RPM |
| [4] | 160 | константа 0xA0 | — |

Примери (потвърдени от capture):
- byte[0]=64 → **0.0 kPa** (минимум — idle без натоварване)
- byte[0]=67 → **1.0 kPa**
- byte[0]=68 → **1.33 kPa**
- byte[3]=20 → **2000 RPM**, byte[3]=31 → **3100 RPM**

Autocom Item 75 = DPF диференциално налягане = param `0x10` byte[0].
...
| 59 | `00 31 00` | `\0 1 \0` |
| 5B | `42 42 41 71 00` | **temp формула**: `(0x42*256+0x42)*0.01-40 = 27.1°C` (IAT?) |
...
| idx | raw bytes | guess |
|-----|-----------|-------|
| 74 | `F3 CC 00 00 FF 60 30` | **EGT**: B0-B9 тип: `(0xF3*256+0xCC)/100 = 624.0°C` (DPF EGT log?) |
...
### Група B0-B9 — **DPF EGT log containers (10 записа × 2 температури)**

Format: `[idx][0x01][A_hi][0x01][A_lo]` — param index + два big-endian uint16 разделени с 0x01.

**Формула: `(A_hi*256 + A_lo) / 100 = °C`**

- Диапазон: 0 до ~655°C (uint16). Покрива EGT при DPF регенерация (400-700°C).
- ECU **пази исторически log** — стойностите не се нулират при изключване, отразяват последната регенерация.
- Потвърдено: smooker е достигал ~700°C при регенерация. Capture без регенерация → стойностите са замразени от последния regen event.
- B0 = (243*256+207)/100 = **624.1°C**, B1 = (232*256+223)/100 = **596.1°C** — типично за активна DPF regen ✓

| idx | A_hi | A_lo | val | °C | бележка |
|-----|------|------|-----|----|---------|
| B0 | F3 | CF | 62415 | 624.1 | EGT log точка 0 |
| B1 | E8 | DF | 59615 | 596.1 | EGT log точка 1 |
| B2-B9 | — | — | — | ~580-630 | (изчисли по формулата) |

**TODO**: capture по време на активна регенерация → да видим дали стойностите се обновяват live или само при край на regen.
